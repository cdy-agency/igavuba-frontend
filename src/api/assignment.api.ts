...
import type {
  ...
  AssignmentSubmissionDetailData,
...
export async function getAssignmentSubmission(submissionId: string) {

  const response = await apiClient.get<

    AssignmentSubmissionMutationResponse<AssignmentSubmissionDetailData>
...
export async function saveAssignmentGrade(
  ...
  >(`/assignments/submissions/${submissionId}/grade`, payload);
...
export async function publishAssignmentGrade(submissionId: string) {

  const response = await apiClient.patch<

    AssignmentSubmissionMutationResponse<PublishGradeResult>

  >(`/assignments/submissions/${submissionId}/publish`);
...
export async function publishAllAssignmentResults(assignmentId: string) {

  const response = await apiClient.patch<

    AssignmentSubmissionMutationResponse<PublishAllResultsData>

  >(`/assignments/${assignmentId}/publish-results`);
