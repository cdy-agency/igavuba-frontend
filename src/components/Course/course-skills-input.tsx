...
export type CourseSkillInputItem = Pick<CourseSkill, 'id' | 'name'> | { name: string };

interface CourseSkillsInputProps {
  items: CourseSkillInputItem[];
  onAdd: (name: string) => void | Promise<void>;
  onRemove: (item: CourseSkillInputItem, index: number) => void | Promise<void>;
  disabled?: boolean;
  isLoading?: boolean;
  error?: string | null;
  isAdding?: boolean;
...
export function CourseSkillsInput({
  items,
  onAdd,
  onRemove,
  disabled = false,
  ...
  return (
    <div className="space-y-3">
      <div>
        <Label className="text-base font-semibold">Skills You&apos;ll Gain</Label>
        <p className="text-sm text-muted-foreground">
          Add the skills learners will develop from this course.
        </p>
      </div>
