    ...
    <div className="bg-surface min-h-screen">
      <div className="flex">
        ...
        <div className="flex-1 p-2 h-screen overflow-y-auto">
          {/* Course Header */}
          <div className="bg-background border border-border mb-1">
            <div className="px-6 py-4 bg-surface border-b border-border flex justify-between items-center">
              <h1 className="text-lg font-medium text-foreground">
                {course.title}
              </h1>
              <div className="flex items-center space-x-3">
                <button className="text-sm text-muted-foreground border border-border px-3 py-1 rounded bg-background hover:bg-surface">
                  Collapse All
                </button>
                <button className="text-sm text-muted-foreground border border-border px-3 py-1 rounded bg-background hover:bg-surface flex items-center gap-2">
                  <Download size={14} />
          ...
          <div className="space-y-0">
            {MOCK_CHAPTERS.map((chapter, index) => (
              <div
                key={chapter.id}
                className="bg-background border-l border-r border-b border-border"
              >
                <button
                  className="w-full text-left px-6 py-4 flex justify-between items-center hover:bg-surface focus:outline-none"
                  onClick={() =>
                ...
                >
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <button
                      className="text-sm text-muted-foreground border border-border px-3 py-1 rounded bg-background hover:bg-surface"
                      onClick={() =>
                        ...
                        <p
                          className={`${
                            chapter.status === "Completed"
                              ? "bg-success/40 p-1 text-xs"
                              : "bg-accent/40 p-1 text-xs"
                          }`}
                        >
                          {chapter.status}
                        </p>
                  ...
                  <div className="border-t border-border bg-surface">
                    <div className="px-8 py-3 space-y-1">
                      {chapter.pages.map((page) => (
                        <button
                          key={page.id}
                          className={`block w-full text-left px-4 py-2 text-sm rounded hover:bg-primary-subtle ${
                            activePage?.id === page.id
                              ? "bg-primary-muted text-primary-active font-medium"
          ...
          {/* Active Page Content */}
          {activePage && (
            <div className="mt-6 bg-background border border-border p-6">
              <h2 className="text-xl font-semibold text-foreground mb-4">
                {activePage.title}
              </h2>
              <div className="text-foreground-muted">{activePage.content}</div>
            </div>
          )}
        </div>

        {/* Sidebar */}
        <div className="w-80 bg-background border-l border-border h-screen overflow-y-auto">
          <div className="p-4 space-y-6">
            {/* Course Actions */}
            <div className="space-y-2">
              <button className="w-full text-left px-3 py-2 text-sm text-foreground-muted hover:bg-surface rounded flex items-center gap-2">
                <Eye size={16} />
                View Course Stream
              </button>
              <button className="w-full text-left px-3 py-2 text-sm text-foreground-muted hover:bg-surface rounded flex items-center gap-2">
            ...
            {/* To Do Section */}
            <div>
              <h3 className="text-sm font-medium text-destructive mb-3">To Do</h3>
              <div className="space-y-2">
                {MOCK_TODO.map((item) => (
                  <div
                    key={item.id}
                    className="border border-border p-3 rounded bg-surface"
                  >
                  ...
                  >
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <div className="flex-1">
                        <div className="text-sm text-primary hover:underline cursor-pointer mb-1">
                          {feedback.title}
                        </div>
                        <div
