...
export function getAssessmentsByCourse(
  courseId: string,
): Promise<ApiResult<Assessment[]>> {
  return request<Assessment[]>(() =>
    axiosInstance.get(`${BASE}/course/${courseId}`),
  );
}

export function getAssessmentsByModule(
  moduleId: string,
): Promise<ApiResult<Assessment[]>> {
  return request<Assessment[]>(() =>
    axiosInstance.get(`${BASE}/module/${moduleId}`),
  );
}
...
export function startAssessmentAttempt(
  assessmentId: string,
): Promise<ApiResult<StartAttemptResponse>> {
  return request<StartAttemptResponse>(() =>
    axiosInstance.post(`${BASE}/${assessmentId}/start`),
  );
}
...
export interface SubmissionWithDetails extends Omit<
  AssessmentSubmission,
  "attempt" | "answers"
> {
  attempt?: {
